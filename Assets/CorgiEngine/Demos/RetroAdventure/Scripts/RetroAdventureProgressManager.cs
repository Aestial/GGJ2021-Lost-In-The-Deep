...
{	
	...
	public class RetroAdventureProgressManager : Singleton<RetroAdventureProgressManager>, MMEventListener<CorgiEngineStarEvent>, MMEventListener<CorgiEngineEvent>
	{
		...
		public int MaximumLives = 0;
		/// the current amount of lives left to the player
		public int CurrentLives = 0;
		/// the list of scenes that we'll want to consider for our game
		public RetroAdventureScene[] Scenes;

		[InspectorButtonAttribute("CreateSaveGame")]
		...
		{
			Progress progress = (Progress)SaveLoadManager.Load(_saveFileName, _saveFolderName);
			if (progress != null)
			{
