	...
	public class CorgiController : MonoBehaviour 
	{
        ...
        protected virtual void MoveTransform()
        {
            if (PerformSafetyBoxcast)
            {
                ...
                if (_stickRaycast)
                {
                    if (Mathf.Abs(_stickRaycast.distance - _newPosition.magnitude) < 0.002f)
                    {
				...
				{	
					...
					if (hitAngle > Parameters.MaximumSlopeAngle)
					{
                        ...
                        if ((_movementDirection == raysDirection) || (CastRaysOnBothSides && (_speed.x == 0f)))
                        {
                            ...
                            if (!State.IsGrounded && (Speed.y != 0) && (hitAngle != 90f))
                            {
                                _newPosition.x = 0;
                            }
		...
		protected virtual void StickToSlope()
        {
				...
				|| (_movingPlatform != null))
			{
                ...
                if (!(!State.WasGroundedLastFrame
                    && ((StandingOnLastFrame != null) && StairsMask.MMContains(StandingOnLastFrame.layer))
                    && !(State.IsJumping) 
                    ))
                {
                    return;
        ...
        protected virtual void ComputeDistanceToTheGround()
        {
            ...
            _distanceToTheGroundRaycast = MMDebug.RayCast(_rayCastOrigin, -transform.up, DistanceToTheGroundRayMaximumLength, _raysBelowLayerMaskPlatforms, MMColors.CadetBlue, false);
