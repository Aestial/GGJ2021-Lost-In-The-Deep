...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		/// by default, the length of the raycasts used to get back to normal size will be auto generated based on your character's height, but here you can specify a different value
		...
		public virtual bool CanGoBackToOriginalSize()
		{
			...
			bool headCheckLeft = MMDebug.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask,Color.cyan,true);

			_originalSizeRaycastOrigin = _boundsTopRightCorner + (Vector2)transform.up * _smallValue;
			bool headCheckRight = MMDebug.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask,Color.cyan,true);
