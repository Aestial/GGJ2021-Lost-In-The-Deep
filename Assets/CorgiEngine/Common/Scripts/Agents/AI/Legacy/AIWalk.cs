...
{	
	...
	public class AIWalk : MonoBehaviour
	{
		...
		[MMInformation("Add this component to a Character and it will walk, turn back when it hits a wall, and try and avoid holes if you ask it to.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		...
		[MMInformation("Decide whether your character should change direction when hitting a wall, and if it should try to avoid holes.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		...
		[MMInformation("If your AI Walk's behaviour is set to Move On Sight, you can define here its 'view' distance (the length of the raycasts used to detect targets), the distance at which it should stop from the target, an offset for the raycast's origin, and the target layer.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		/// The maximum distance at which the AI can see the target
		[Tooltip("The maximum distance at which the AI can see the target")]
		...
		[Tooltip("the horizontal distance from its target at which the agent will stop moving. Between that distance and the walk distance, the agent will slow down progressively")]
		...
		[Tooltip("the layer mask to use to detect targets")]
